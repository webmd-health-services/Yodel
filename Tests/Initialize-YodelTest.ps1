...
.DESCRIPTION
The `Initialize-YodelTest.ps1` script gets your tests ready to run by:
...
try
{
    $modules = [ordered]@{
        'Yodel' = '..\Yodel';
        'YodelTestHelper' = 'YodelTestHelper';
    }
    foreach( $moduleName in $modules.Keys )
    {
        ...
        if( $module )
        {
            ...
            if( (Test-Path -Path 'env:APPVEYOR') -and $module.Path.StartsWith($modulePath) )
            {
                continue
            }
